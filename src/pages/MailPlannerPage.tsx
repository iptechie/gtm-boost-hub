              ...
              <div className="md:w-2/3">
                <div className="glass-card p-6">
                  ...
                  <div className="space-y-4">
                    {activities.filter(activity => new Date(activity.date).toDateString() === date?.toDateString()).map(activity => (
                      <div key={activity.id} className="p-4 border border-slate-200 rounded-lg hover:border-blue-300 transition-colors">
                        <div className="flex justify-between">
                          <h4 className="font-medium">{activity.name}</h4>
                          {getStatusBadge(activity.status)}
                        </div>
                        <p className="text-sm text-slate-500 mt-1">{activity.type}</p>
                    ...
                    {activities.filter(activity => new Date(activity.date).toDateString() === date?.toDateString()).length === 0 && (
                      <p className="text-center text-slate-500 py-4">No activities scheduled for this date</p>
                    )}
                  </div>
                </div>
              </div>
                    ...
                    <TableRow key={activity.id} className="table-row-hover">
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button size="sm" variant="outline" className="text-red-500" onClick={() => toast.info("Delete feature coming soon")}>
                            Delete
                          </Button>
                        </div>
                      </TableCell>
